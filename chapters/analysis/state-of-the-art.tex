...
\todo{Add mongo db query example}
...
\todo{Add mongo typeclass example}

In addition to coding Typeclasses, the driver also expose type-safe interface to build \gls{crud} queries.
These queries are created using simple \gls{dsl} as shown in example (??).

\todo{Add mongo dsl example}
